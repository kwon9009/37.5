...
class Settings(BaseSettings):
    ...
    #
    # 2026-08-12: 기본값을 끔으로 바꿨다.
    # 지금 모델은 정상 생체값에도 "정상" 판정을 내지 못한다. NEWS2 기준 100% 정상인
    # 값(심박 65~82 / 호흡 13~20)을 넣어보면 이상 점수가 최소 53.3 · 중앙 73.2로
    # 40 미만이 한 건도 없고, 규칙상 정상인 측정의 약 73%를 주의·경고로 올린다.
    # 학습 데이터가 합성 3명(심박 70·74·78 / 호흡 15·16·17)뿐이라 모델이 아는
    # 정상이 그 좁은 띠밖에 없고, 처음 보는 환자는 전부 이상으로 보기 때문이다.
    # 실측 데이터로 다시 학습해 정상이 정상으로 나오는 것을 확인한 뒤 다시 켠다.
