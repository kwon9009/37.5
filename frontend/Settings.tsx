...
import { Heart, Wind, UserCheck, UserX, Phone, BellRing, FileText, X } from "lucide-react"
...
function VitalCard({
  ...
  return (
    <div className="flex flex-col rounded-3xl border border-border bg-card p-4 shadow-sm">
...
export default function Home() {
  ...
  return (
    <Screen>
      <TopBar title="37.5°C" bell bellCount={notifications.length} />
      <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
        {/* Patient summary */}
        <div className="flex items-center justify-between rounded-3xl bg-primary p-5 text-primary-foreground">
          <div>
            <p className="text-sm text-primary-foreground/80">모니터링 중인 환자</p>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4 text-left shadow-sm">
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-accent/15 text-accent">
                <BellRing size={20} aria-hidden />
              </span>
              <span className="font-semibold text-foreground">간병인 호출</span>
            </button>
          </div>
        </div>
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block font-semibold text-foreground">특이사항 확인</span>
            <span className="block truncate text-sm text-muted-foreground">{specialNote}</span>
          </span>
        </button>
        ...
        {urgentCount > 0 && (
          <button
            ...
            className="w-full rounded-2xl bg-danger/10 p-4 text-left"
          >
